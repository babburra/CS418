...
float eye[] = {0.5, 0.5, -0.15}; //initial eye vector
...
float dot(float *x, float *y) { // cross product
	return x[0] * y[0] + x[1] * y[1] + x[2] * y[2];
}

float* cross(float *x, float *y) {
	float z[] = { x[1] * y[2] - x[2] * y[1], 
				  x[2] * y[0] - x[0] * y[2], 
				  x[0] * y[1] - x[1] * y[0] };
	return z;
}
...
void rotate(float *x, float *y, float angle) {
	float length = pow(x[0], 2) + pow(x[1], 2) + pow(x[2], 2);

	y[0] = ((x[0] * dot(x, y)) * (1 - cos(angle)) +
		(length * y[0] * cos(angle)) +
		(sqrt(length) * (-x[2] * y[1] + x[1] * y[2]) * sin(angle))) / length;

	y[1] = ((x[1] * dot(x, y)) * (1 - cos(angle)) +
		(length * y[1] * cos(angle)) +
		(sqrt(length) * (x[2] * y[0] - x[0] * y[2]) * sin(angle))) / length;

	y[2] = ((x[2] * dot(x, y)) * (1 - cos(angle)) +
		(length * y[2] * cos(angle)) +
		(sqrt(length) * (-x[1] * y[0] + x[0] * y[1]) * sin(angle))) / length;

}
void yaw(float yawAngle) {
	rotate(up, lookat, yawAngle);
}

void roll(float rollAngle) {
	rotate(lookat, up, rollAngle);
}

void pitch(float pitchAngle) {
	float z[] = { 0.0, 0.0, 0.0 };
	z[0] = lookat[1] * up[2] - lookat[2] * up[1];
	z[1] = lookat[2] * up[0] - lookat[0] * up[2];
	z[2] = lookat[0] * up[1] - lookat[1] * up[0];
	rotate(z, lookat, pitchAngle);
	z[0] = lookat[1] * up[2] - lookat[2] * up[1];
	z[1] = lookat[2] * up[0] - lookat[0] * up[2];
	z[2] = lookat[0] * up[1] - lookat[1] * up[0];
	rotate(z, up, pitchAngle);
}

void moveForward() {
	eye[0] += velocity * lookat[0];
	eye[1] += velocity * lookat[1];
	eye[2] += velocity * lookat[2];
}
...
void display(void)
{
	...
	gluLookAt(eye[0], eye[1], eye[2], lookat[0] + eye[0], lookat[1] + eye[1], lookat[2] + eye[2], up[0], up[1], up[2]);
	moveForward();
...
void reshape(int w, int h)
{
	...
	glLoadIdentity();
	gluPerspective(100.0, (float) w / h, 0.01, 10.0); // first parameter: up and down  second: right tilt and left tilt
